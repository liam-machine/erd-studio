...
All tools are read-only. Mutation tools (propose model change, apply sync plan) are planned for v2.
...
If your dbt project doesn't have an `erd-studio/` directory yet, install the extension and run `dbt: Set Up Semantic Domains Directory` in VS Code. The MCP server reads `manifest.json`-only data too (via `list_manifest_models`), so you can start there before drawing any ERDs.
