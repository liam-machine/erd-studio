...
/**
 * How an attachment entered the dialog.
 *
 * Only the canvas capture remains: GitHub has no API for putting an image on a
 * prefilled issue form, so an image can only reach the issue through the user's
 * own clipboard or by them dropping it on the GitHub page. The extension can do
 * the canvas capture better than the user can by hand — right crop, one click,
 * already on the clipboard — and a file they already have on disk it cannot, so
 * that route is handed back to GitHub rather than half-served here.
 */
export type FeedbackAttachmentSource = 'canvas';

export const FEEDBACK_ATTACHMENT_SOURCES: readonly FeedbackAttachmentSource[] = ['canvas'];

/**
 * Maximum number of images on one report. One: the canvas capture, which is
 * the single image route the dialog owns.
 */
export const MAX_ATTACHMENTS = 1;

/** Maximum decoded size of that image (10 MB). postMessage carries it as base64, so the wire cost is ~4/3 of this. */
...
export interface FeedbackAttachment {
  ...
  id: string;
  /** File name the image is saved under — `canvas.png` for the captured canvas. */
  name: string;
...
/**
 * The denominator when every check applies — a bug filed over a live canvas.
 *
 * `readinessScore()` computes its own denominator from the checks it actually
 * applied, because the image check only exists where an image route does (see
 * `ReadinessInput.canAttachImage`). This constant is that full-house case, and
 * the sum the weights are asserted against.
 */
...
export interface ReadinessInput {
  ...
  /** 0 or 1 — the canvas capture is the only image the dialog attaches. */
  attachmentCount: number;
  includeDiagnostics: boolean;
  /**
   * Whether this report has any way to attach an image at all.
   *
   * The canvas capture is the only route left, and it is bug-only and needs a
   * canvas behind the dialog. Where there is no route, the image check is not
   * applied and its weight leaves the denominator — otherwise a feature request
   * would be capped at 80% by a check it can never meet, under a button that
   * could do nothing. Defaults to `kind === 'bug'`; pass `false` for a bug with
   * no canvas open.
   */
 ...
 * otherwise the line explains how the image reaches GitHub, because that
 * hand-off is the only part of the flow the user has to finish by hand.
 */
export function footerRoute(input: FooterRouteInput): FooterRoute {
  ...
  // At most one image, so there is no "the first of N" case to explain.
...
export function readinessScore(input: ReadinessInput): ReadinessResult {
  ...
  let points = 0;
  // Only the checks that were applied count towards the denominator, so a
  // report with no image route still reaches 100 when it is as complete as it
  // can be.
  let max = READINESS_WEIGHTS.description + READINESS_WEIGHTS.context + READINESS_WEIGHTS.diagnostics;
  ...
  // An image is only asked for where one can be attached. On a feature request
  // (or a bug with no canvas open) the only route — the canvas capture — does
  // not exist, so the check is skipped entirely rather than standing there
  // unmeetable above a button with nothing to do.
  const canAttachImage = input.canAttachImage ?? input.kind === 'bug';
  if (input.attachmentCount > 0) {
    points += READINESS_WEIGHTS.attachment;
    max += READINESS_WEIGHTS.attachment;
    checks.push({ ok: true, label: 'Screenshot attached' });
