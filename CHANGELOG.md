...
- **One-click canvas screenshot.** Tick the box on a bug report and the canvas behind the dialog — the domain and stage are named next to it — is captured and put straight on your clipboard, ready to paste into the issue's Screenshot box. If your clipboard refuses it, the PNG is still saved to a folder the notification can reveal, so the screenshot is never lost. Any other images are added on the GitHub page itself, which the dialog says rather than offering a picker whose files it could not deliver.
...
- **Copy report.** Copies the whole report — title, kind, description, steps or rationale, the attached screenshot's name and diagnostics — as Markdown, so it can go into an email, a chat or your own tracker instead. Works with no network, no GitHub account and no model.
...
- **The readiness meter no longer asks a feature request for a screenshot it cannot take.** The canvas capture is the dialog's only image route and it is bug-only, so "No image — Attach one" sat permanently unmet on every feature request, capping the bar at 80% under a button that did nothing. The image check now applies only where a capture is actually available (a bug, with a canvas open), and the remaining checks are scored out of what is left — a complete feature request reads 100%.
