...
function AnnotationNodeInner({ data }: NodeProps<AnnotationFlowNode>) {
  ...
  }, [localText, data.text, data.annotationId, host, setEditingAnnotationId]);
  ...
  const handleColorChange = useCallback(
    (color: AnnotationColor) => {
      host.postMessage({ type: 'updateAnnotation', payload: { id: data.annotationId, color } });
      setToolbarOpen(false);
    },
    [data.annotationId, host],
  );
  ...
  const handleDelete = useCallback(() => {
    host.postMessage({ type: 'removeAnnotation', payload: { id: data.annotationId } });
    setToolbarOpen(false);
  }, [data.annotationId, host]);
  ...
  const handleResizePointerUp = useCallback(
    ...
    [resizing, data.annotationId, host],
  );
