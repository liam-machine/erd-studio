...
| `erdStudio.feedback.provider` | `auto` | Which destination does the analysis: `auto` (the original precedence), `vscode`, `endpoint` or `hosted`. Any value but `auto` **pins** one tier and does not fall through — an unavailable pin resolves to `none` rather than sending the text somewhere the user did not pick. Set from the dialog's own picker via `setFeedbackProvider`. |
