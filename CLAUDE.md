...
3. **DomainService** reads a domain JSON → `UnifiedDomain`, resolving each model name through **LogicalModelService** (`logical-models/{name}.yml`, parsed with `yaml` `parseDocument` so comments/key order survive round-trips; a missing file renders as a placeholder with a warning), then extracts a stage section via `getDomainStage()` → `DisplayDomain`. Malformed `viewConfig.positions` entries and relationships missing endpoints are dropped with a console warning.
...
Own writes are recorded in `ownWrites` (`src/services/ownWriteTracker.ts`, path + mtime/size, 5 s TTL) by `LogicalModelService`, `LayerService` and domain deletes, and consumed in every debounced callback so an extension save never triggers a second identical refresh. `classifySemanticPath()` ensures only `{layer}/{domain}.json` fires `onSemanticFileDeleted` (coalesced to one `{ uris }` event per burst); `layers.json` has its own `onLayerConfigChanged`. `LayerService.loadConfig()` is mtime-validated and refuses to overwrite an unreadable/invalid `layers.json` (`getLoadError()`).
