...
1. Sign the **Eclipse Publisher Agreement**. It needs two accounts linked together. First create an Eclipse Foundation account at https://accounts.eclipse.org/user/register and fill in its **GitHub Username** field with the exact GitHub account you use on open-vsx.org (an existing Eclipse account can add it under Edit Profile). Then on https://open-vsx.org sign in with GitHub, open the avatar menu → **Settings → Profile**, click **Log in with Eclipse** and authorise it. Only after that link exists does the profile page show **Show Publisher Agreement**; read to the bottom and click **Agree**. Publishing is refused until the agreement is signed.
