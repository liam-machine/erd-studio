...
describe('ManifestService', () => {
  ...
  describe('loadManifest', () => {
    it('parses a sample manifest and returns model data', async () => {
      ...
      // Should have 3 models (dim_task, dim_project, fct_task_event)
    ...
    it('extracts model metadata correctly', async () => {
      ...
      expect(model!.uniqueId).toBe('model.my_dbt_project.dim_task');
  ...
  describe('hasModel', () => {
    it('returns true for existing models', async () => {
      ...
      expect(service.hasModel('dim_task')).toBe(true);
      expect(service.hasModel('dim_project')).toBe(true);
      expect(service.hasModel('fct_task_event')).toBe(true);
    });
  ...
  describe('invalidate', () => {
    ...
    it('clears cached model lookups', async () => {
      ...
      expect(service.hasModel('dim_task')).toBe(false);
      expect(service.getModelColumns('dim_task')).toEqual([]);
  ...
  describe('relationship test extraction (F409)', () => {
    ...
    it('parses relationship test fields correctly', async () => {
      ...
      // Find the dim_task → dim_project relationship test
      const relTest = data.relationshipTests.find(
        (r) => r.fromModel === 'dim_task' && r.toModel === 'dim_project'
      );
    ...
    it('extracts fromModel from attached_node when available', async () => {
      ...
      const relTest = data.relationshipTests.find(
        (r) => r.fromModel === 'fct_task_event'
      );

      expect(relTest).toBeDefined();
      expect(relTest!.fromModel).toBe('fct_task_event');
      expect(relTest!.toModel).toBe('dim_task');
    });
